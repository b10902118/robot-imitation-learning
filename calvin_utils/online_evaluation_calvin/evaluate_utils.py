...
import calvin_utils.pytorch3d_transforms as pytorch3d_transforms
from calvin_utils.utils_with_calvin import (
    convert_rotation
)
...
def prepare_visual_states(obs: Dict[str, Dict[str, Any]],
    ...
    if 'obs' not in obs:
        ...
        obs['dp_obs']['rgb_obs']['rgb_static'] = np.concatenate([
            obs['dp_obs']['rgb_obs']['rgb_static'][1:], rgb_static[None]
        ], axis=0)
        obs['dp_obs']['rgb_obs']['rgb_gripper'] = np.concatenate([
            obs['dp_obs']['rgb_obs']['rgb_gripper'][1:], rgb_gripper[None]
        ], axis=0)
